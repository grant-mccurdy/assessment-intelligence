WITH score_pairs AS (
    ...
    JOIN analytics.dim_student AS ds
...
SELECT
    ...
    ROUND(AVG(assignment_01_score)::numeric, 2) AS assignment_01_avg,
    ROUND(AVG(assignment_02_score)::numeric, 2) AS assignment_02_avg,
    ROUND(AVG(observed_growth_delta)::numeric, 2) AS avg_observed_growth_delta,
    ROUND(MIN(observed_growth_delta)::numeric, 2) AS min_observed_growth_delta,
    ROUND(MAX(observed_growth_delta)::numeric, 2) AS max_observed_growth_delta
FROM growth
GROUP BY
    grade_level,
    course_id,
