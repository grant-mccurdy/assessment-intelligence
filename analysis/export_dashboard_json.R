...
source_metadata <- function(student_records, summary, sections) {
  list(
    project = "assessment-intelligence",
    pipeline = "analysis/run_pipeline.R",
    exporter = "analysis/export_dashboard_json.R",
    syncTarget = "grant-mccurdy.github.io/data/synthetic/assessment-dashboard.json",
    contract = "static-assessment-dashboard-json-v1",
    dataClass = "public-safe synthetic assessment data",
    recordCounts = list(
      periods = length(unique(summary$periodId)),
      sections = nrow(sections),
      aggregateRecords = nrow(summary),
      syntheticStudentRecords = nrow(student_records)
    )
  )
}
...
build_dashboard <- function(student_records, summary, sections, calibration) {
  ...
  list(
    ...
    source = source_metadata(student_records, summary, sections),
    description = paste(
